...
public function searchData($searchTerm){
    try {
        // Initialize arrays to store results for questions, assignments, and notices
        $questionResults = [];
        $assignmentResults = [];
        $noteResults = [];
        $noticeResults = [];
        
        $searchTerm = $this->sanitize_input_data($searchTerm);

        // Search for questions
        $sqlQuestions = "SELECT * FROM swastik_questions WHERE q_content LIKE :searchTerm";
        $stmtQuestions = $this->db->prepare($sqlQuestions);
        $searchTerm = "%{$searchTerm}%"; // Add wildcard for partial matching
        $stmtQuestions->bindParam(':searchTerm', $searchTerm, PDO::PARAM_STR);
        $stmtQuestions->execute();
        
        // Fetch and store question results
        $questionResults = $stmtQuestions->fetchAll(PDO::FETCH_ASSOC);
        
        // Search for assignments
        $sqlAssignments = "SELECT * FROM swastik_assignments WHERE assi_title LIKE :searchTerm";
        $stmtAssignments = $this->db->prepare($sqlAssignments);
        $stmtAssignments->bindParam(':searchTerm', $searchTerm, PDO::PARAM_STR);
        $stmtAssignments->execute();
        
        // Fetch and store assignment results
        $assignmentResults = $stmtAssignments->fetchAll(PDO::FETCH_ASSOC);
        
        // Search for notes
        $sqlNotes = "SELECT * FROM swastik_notes WHERE note_title LIKE :searchTerm";
        $stmtNotes = $this->db->prepare($sqlNotes);
        $stmtNotes->bindParam(':searchTerm', $searchTerm, PDO::PARAM_STR);
        $stmtNotes->execute();
        
        // Fetch and store note results
        $noteResults = $stmtNotes->fetchAll(PDO::FETCH_ASSOC);
        
        // Search for notices
        $sqlNotices = "SELECT * FROM swastik_notices WHERE notice_content LIKE :searchTerm";
        $stmtNotices = $this->db->prepare($sqlNotices);
        $stmtNotices->bindParam(':searchTerm', $searchTerm, PDO::PARAM_STR);
        $stmtNotices->execute();
        
        // Fetch and store notice results
        $noticeResults = $stmtNotices->fetchAll(PDO::FETCH_ASSOC);

        // Combine the results into a single array with labels
        $results = [
